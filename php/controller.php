...
if(isset($_POST['add_patient'])){ 
	...
	//echo $gender+ $birthday +$gender+ $address;
	// echo $birthday;
	// echo $gender;
	// echo $img;
	// echo $target_file;
	//'', 'ritu raut', '1997-11-05', 'Female', 'add', '9999999999', 'c.jpg', 'ritu@gmail.com', 'in', 'none', '1'
    move_uploaded_file($_FILES["img"]["tmp_name"], $target_file);   
	    if(udi("$i patients (name, birthday, gender, address, phone, img, email, confine, room, status) values ('$name','$birthday','$gender','$address','$phone','$img', '', '', '', '1')")){
	    	$notifications = "New patient has been added to records !";
	    	$task = "<script>$('#notifications').click();</script>"; 
	    }
		// if(udi("$i patients values('29', 'ritu raut', '1997-11-05', 'Female', 'add', '9999999999', 'c.jpg', 'ritu@gmail.com', 'in', 'none', '1')")){
	    // 	$notifications = "New patient has been added to records !";
	    // 	$task = "<script>$('#notifications').click();</script>"; 
	    // } 
		else { 
...
if(isset($_POST['add_physician'])){ 
	...
	$target_file = $target_dir.basename($_FILES["img"]["name"]); 
    move_uploaded_file($_FILES["img"]["tmp_name"], $target_file);   
	    if(udi("$i physician (name, phone, fee, img, status, liscense, email, bday, address) values('$name','$phone','$fee', '$img','1', '32321', '$email', '$bday', '$address')")){
...
if(isset($_POST['newtransaction'])){ 
	$patient_id = filters('patient_id'); 
	$physician_id = filters('physician_id'); 
		if(udi("$i transactions  (date, patient_id, physician_id, total, payment_status, status) values(CURRENT_TIMESTAMP,'$patient_id','$physician_id','0','0','1')")){
